...
All observations below come from **three** consecutive `Get-Content -Wait
-Tail 20` capture sessions on a real Windows machine, playing actual Public
Match online co-op. Nothing here is speculative unless explicitly marked
"OPEN QUESTION" or "UNCONFIRMED." Session-specific findings are labeled
where relevant (Session 1/2 = original two captures; Session 3 = a capture
covering a full game launch → join an existing public lobby → shutdown).
...
- Game version observed in this session: `Binding of Isaac: Repentance+ v1.9.7.17.J460`
  (also appears as `Game Version: J460` on a separate line). Same version
...
- **Session 3 confirms the file is reused/truncated across full game
  restarts**: a Session 3 capture began mid-file with the tail end of a
  *previous* run's shutdown sequence still visible (see §3.8), followed by
  a brand new `load archives:` boot sequence for the next launch. A parser
  tailing this file live must handle the file being truncated/recreated
  when the game restarts, not just appended to.
...
Same mod list loads identically in Session 3, no change.
...
| `Successfully joined lobby <LOBBYID>` | `Successfully joined lobby 109775242298099731` | Fires both when creating your own lobby (immediately after "Successfully created lobby") AND when joining someone else's existing lobby. This is the reliable "I am now in lobby X" signal regardless of host/guest role. Session 3 captured a clean example of the guest-joins-existing-public-lobby case in isolation (see §3.3). |
...
directly in the Session 1/2 sample (joining lobby `109775242300673142`
showed no join lines for `DECADENT`, `activa_tore`, or `TToshtet`/`Romash`),
and **confirmed again independently in Session 3**: after
`Successfully joined lobby 109775242306538330`, none of the three
pre-existing members (`Valar Morghulis`, `MaXzl`, `Luks...`) produced a
join line — they only appeared via the heartbeat pattern below. Two
independent sessions now agree on this behavior.
...
| `[Frame <FRAME>] Received Run Status message from user <NAME> [<STEAMID>]` | `[Frame 0] Received Run Status message from user Valar Morghulis [76561198083258092]` | **New in Session 3, not seen in Sessions 1/2.** Same `NAME [STEAMID]` format as Player Info/Save Data. Observed firing multiple times in a row for the same user (e.g. 4× for `Luks...` in quick succession), similar to Player Info's repeating/heartbeat behavior rather than Save Data's one-shot behavior. Fired for all three pre-existing lobby members shortly after joining. OPEN QUESTION: exact semantic difference from "Player Info" (presumably carries current-run status — e.g. alive/dead, which floor — rather than general presence, but the log gives no further detail to confirm this). Functionally, for lobby-roster purposes, it's redundant with Player Info (same name+ID pair) and doesn't need separate handling by a tracker, but is documented here so it isn't mistaken for an unknown/unhandled line type. |
...
event. "Received Run Status" (Session 3) appears to follow the same
repeating pattern.

**Session 3 clean join-sequence example**, immediately after joining an
existing public lobby as guest (no lobby creation on the local side):
```
[INFO] - Successfully joined lobby 109775242306538330
[INFO] - Menu_OnlineLobby::broadcast_lobby_info_message() broadcasting lobby info message
[INFO] - [Frame 0] Received Player Info message from user Valar Morghulis [76561198083258092]
[INFO] - [Frame 0] Received Save Data message from user Valar Morghulis [76561198083258092]
[INFO] - [Frame 0] Received Run Status message from user Valar Morghulis [76561198083258092]
[INFO] - [Frame 0] Received Player Info message from user MaXzl [76561199726068308]
[INFO] - [Frame 0] Received Save Data message from user MaXzl [76561199726068308]
[INFO] - [Frame 0] Received Run Status message from user MaXzl [76561199726068308]
[INFO] - [Frame 0] Received Player Info message from user Luks... [76561198761191603]
[INFO] - [Frame 0] Received Save Data message from user Luks... [76561198761191603]
[INFO] - [Frame 0] Received Run Status message from user Luks... [76561198761191603]
```
Each existing member announces itself as a `Player Info` → `Save Data` →
`Run Status` triplet, in that order, once per member, before settling into
the repeating heartbeat. This triplet ordering held for all three members
in this sample and may be a reliable "new member fully synced" signal
worth testing further (UNCONFIRMED as a general rule from a single
sample).
...
**Timing caveat**: in the Session 1/2 sample, `Local user ID = ...` only
appeared once, and only *after* a run had ended (post "Game Over" / post
"NotifyGameEnd"), not immediately at lobby creation or game start.
**Session 3 did not reach a run/game-start at all** (lobby joined, then the
game was closed straight from the lobby menu) and correspondingly showed
**no `Local user ID` line anywhere in that capture**. This is consistent
with — and further supports — the standing hypothesis that this line only
appears after a run completes, not merely after joining a lobby. Still not
100% confirmed as a hard rule (no session has yet captured a lobby that
was joined, never played, and also somehow produced this line), but two
sessions now agree it's absent without a completed run.
...
2. A large block of engine/asset reinitialization follows — OpenGL/OpenAL
   ...
   **This exact block (steps 1-2, minus the `send_game_start` line) is
   also what a fresh game *launch* looks like from cold boot** — Session 3
   confirms the "engine reinit + shader init + mod list + menu init +
   persistent data load" sequence is identical whether it's triggered by
   starting a run from a lobby or simply launching the executable from
   scratch. The two contexts are distinguishable by what immediately
   precedes the block: `send_game_start()` for the in-lobby case vs.
   nothing/a prior process start for a cold launch (see §3.8).
3. `[Frame <FRAME>] Received Join New Game message from user <NAME> [<STEAMID>]`
   ...
   *other* party member when the run actually starts (i.e., is it a
   ...
   Needs a multi-player-at-game-start sample to confirm. **Still
   unconfirmed after Session 3**, which never reached a game start.
...
in the Session 1/2 sample, lobby `109775242301907426` was created, a run
was played and ended (death), and afterward two NEW players (`raligarh`,
...
### 3.8 Application shutdown sequence — NEW, Session 3

Not previously documented. Session 3 captured the tail end of a *previous*
game process's shutdown at the very start of the log excerpt (i.e., this
is what precedes a fresh `load archives:` boot line when the game is
closed and relaunched), and again captured a full shutdown at the very end
of the same session after joining a lobby.

Observed sequence, in order:

| Pattern | Example | Notes |
|---|---|---|
| `Isaac is shutting down...` | as-is | First and most reliable signal that the process is closing. Fired both from the main menu/lobby screen (Session 3, no run in progress) and was inferred present at the boundary between the two boot sequences at the top of the Session 3 capture. **Best single line to trigger "clear all lobby/session state" on.** |
| `Saving PersistentGameData to Steam Cloud: rep+persistentgamedata1.dat.` | as-is | Final save-on-exit. Same message format as the periodic in-session saves documented implicitly elsewhere (not previously called out as its own row) — this occurrence specifically is the exit-time save, distinguishable only by position (immediately after the "shutting down" line), not by content. |
| `CPU time spent on enemy queries: 0.00 seconds.` | as-is | Diagnostic/telemetry line, fires once at shutdown. `0.00` seen when closing from a lobby/menu with no active room; UNCONFIRMED whether this reflects real accumulated time when closing mid-run. Not participant-related. |
| `Isaac has shut down successfully` | as-is | Final line of the process's log output before the file would next be truncated/reused by a subsequent launch. |

**Relationship to lobby state**: Session 3 shows the shutdown sequence
firing while still nominally "in" a joined lobby (no `Leaving current
lobby` line appeared beforehand) — the process simply exits with the
lobby membership still logically active from the log's perspective. This
confirms a tracker cannot rely on `Leaving current lobby` alone to know
when to clear its roster; it must **also** treat `Isaac is shutting down`
as an unconditional "clear everything, session is over" signal,
independent of whatever lobby/leave state was last observed.

**Boot sequence immediately following a shutdown** (i.e., next launch) is
identical to the block described in §3.5 step 2 / §1, starting again from
`load archives: <N> milliseconds` and the shader init block — no special
"first launch of the day" markers were observed distinguishing a cold boot
from any other cold boot.
...
- **NEW (Session 3) — non-ASCII / emoji display names**: one observed name
  (`Luks<emoji>`) contained what appears to be an emoji character that
  rendered as mojibake (garbled multi-byte sequence, e.g. `рџ‘Ќ`-style
  bytes) in the captured log text, rather than the actual emoji glyph.
  This suggests the log file's on-disk encoding does not cleanly round-trip
  all Unicode display names — likely a UTF-8 vs. Windows-1252/CP1252
  mismatch somewhere in the game's own logging pipeline, not an artifact
  of the capture method. Practical implication for a parser: **read the
  log file with lenient/replace-on-error decoding** (don't hard-fail or
  drop lines on decode errors), and don't assume a display name is safe to
  use as-is in contexts that need clean text (e.g. don't build file paths
  or UI labels from it without sanitizing) — but the surrounding regex
  structure (`from user (.+?) \[(\d+)\]`) still matched correctly around
  the garbled bytes in practice, since it doesn't need to understand the
  name's content, just find its boundaries.
...
1. Does `[Frame 0] Received Join New Game message from user ...` fire once
   ...
   **Still open after Session 3** (no game-start reached).
...
6. Does `Menu_OnlineLobby::broadcast_lobby_info_message()` burst frequency
   ...
   periodic/unrelated?
7. **NEW**: Is the `Player Info` → `Save Data` → `Run Status` triplet
   ordering (§3.3) reliable across sessions as a "member fully synced"
   signal, or was the consistent ordering in Session 3 a coincidence of
   a 3-member sample?
8. **NEW**: What specifically does `Received Run Status message` carry
   semantically (current floor? alive/dead state? something else)? The
   log gives no visible payload beyond name+ID, so this can only be
   answered by correlating its firing times against known in-run events
   in a future sample, or from REPENTOGON/game source if available.
9. **NEW**: Does `Isaac is shutting down...` ever fail to fire on a
   non-graceful exit (crash, `taskkill`, power loss)? Only graceful exits
   have been observed so far; a tracker relying on it to clear state
   should still have a secondary fallback (e.g. detecting the log file's
   process handle closing, or a timeout-based staleness check) for
   ungraceful terminations, which is UNCONFIRMED to produce this line.
